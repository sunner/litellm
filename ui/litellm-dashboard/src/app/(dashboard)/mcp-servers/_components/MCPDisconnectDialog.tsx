  ...
  const handleConfirm = async () => {
    ...
    try {
      if (scope === "server") {
        ...
        NotificationsManager.success("Cleared your connection to this MCP server");
      }
      onOpenChange(false);
      onCleared(scope);
    } catch (error) {
      NotificationsManager.fromBackend(error instanceof Error ? error.message : "Failed to clear the MCP OAuth token");
    } finally {
